...
export async function checkOAuthCallback(): Promise<OAuthSession | null> {
  const tabs = await chrome.tabs.query({ url: CALLBACK_URL_PATTERN })
  for (const tab of tabs) {
    ...
    const { session } = await client.initCallback(new URLSearchParams(p.toString()), OAUTH_CALLBACK_URL)
    const callbackTabs = await chrome.tabs.query({ url: CALLBACK_URL_PATTERN })
    await Promise.all(callbackTabs.filter(t => t.id != null).map(t => chrome.tabs.remove(t.id!)))
    return session
  }
  return null
}
